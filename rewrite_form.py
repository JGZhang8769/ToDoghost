...
# Instead of relying on a giant exact match, use regex to target the form content

match = re.search(r'(<h2 class="font-bold mb-4 shrink-0">{{ editingTask\?\.id \? \'編輯代辦\' : \'新增代辦\' }}</h2>\s*<div class="overflow-y-auto flex-1 pb-4">)(.*?)(</div>\s*<div class="flex gap-2 justify-end mt-4 shrink-0 pt-2 border-t border-milktea-100">)', content, re.DOTALL)
...
if match:
    ...
    new_middle = """
          ...
          """
    content = content[:match.start(2)] + new_middle + content[match.start(3):]
