...
if (!fs.existsSync(SERVICE_ACCOUNT_FILE)) {
  ...
  console.error(
    'And save it as "service-account.json" in the todoghost folder.',
  );
  console.error("===========================================================");
  process.exit(1);
}
...
async function checkTasksAndNotify() {
  try {
    ...
    console.log(
      `System current time: ${currentYear}-${String(currentMonth).padStart(2, "0")}-${String(currentDay).padStart(2, "0")} ${String(currentHour).padStart(2, "0")}:${String(currentMinute).padStart(2, "0")}`,
    );
    ...
    for (const taskDoc of tasksSnapshot.docs) {
      ...
      if (!task.date || !task.startTime || task.reminderOffset == null)
        continue;
      ...
      if (isDateMatch && isTimeMatch) {
        console.log(
          `[HIT] Task "${task.title}" is due NOW (${targetStr})! Sending push to user ${userId}...`,
        );
        ...
        console.log(
          `[DEBUG] Task "${task.title}": Reminder targets ${targetStr} (System is currently ${currentHour}:${currentMinute})`,
        );
      }
    }
...
function scheduleNextMinute() {
  ...
  console.log(
    `[INIT] Aligning clock. Waiting ${Math.round(delay / 1000)} seconds until the next exact minute...`,
  );
