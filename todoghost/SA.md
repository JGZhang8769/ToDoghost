...
- **`UserService`**: 負責處理使用者的儲存、讀取與狀態維護。支援 WebAuthn (Face ID/Touch ID) 註冊與驗證流程，以及 Fallback 的 PIN 碼登入。
...
- **`MainViewComponent`**: 主要的行事曆與代辦操作介面。包含：
  - 月視圖 (自動根據時間排序)
  - 週視圖 (直式時間軸以利手機閱讀，自動根據時間排序)
  - 日視圖 (改進了全天區塊與排程重疊顯示，支援左右滑動)
  - 底部浮動抽屜 (絕對定位不影響主體滾動，相容 iOS safe-area)
  - 全域拖曳垃圾桶 (只有在拖曳時顯示)

## 4. 資料庫結構 (Firestore)

- **`users`**: `{ id: string, name: string, avatar: string, pin: string }`
