...
@Component({
  ...
  template: `
    <div class="flex flex-col h-full bg-milktea-50">
      <!-- Header -->
      <div
        class="bg-white px-4 py-3 shadow-sm border-b border-milktea-100 flex items-center relative"
      >
        <button
          (click)="goBack()"
          class="text-milktea-400 p-2 -ml-2 whitespace-nowrap absolute left-4"
        >
          ← 返回
        </button>
      ...
      <div class="flex-1 p-4 overflow-y-auto">
        <div
          class="bg-white rounded-xl shadow-sm border border-milktea-100 p-4 mb-4 cursor-pointer hover:bg-milktea-50 transition-colors"
        ...
        >
          ...
          <p class="text-xs text-milktea-500 mt-2 pl-9">
            設定與排序空間的自訂任務分類清單
          </p>
        </div>
        ...
        >
          <div class="flex justify-between items-center">
            <div class="flex items-center gap-3">
              <span class="material-icons text-milktea-500"
      ...
      <div
        *ngIf="showFeatureDrawer"
        class="fixed inset-0 bg-black/20 z-[60] transition-opacity"
        (click)="closeFeatureDrawer()"
      ></div>
      <div
        class="fixed bottom-0 left-0 right-0 bg-white rounded-t-3xl shadow-[0_-4px_15px_rgba(0,0,0,0.1)] transition-transform duration-300 z-[65] max-w-3xl mx-auto flex flex-col"
        [style.transform]="
          showFeatureDrawer ? 'translateY(0)' : 'translateY(100%)'
        "
      >
        <div
          class="p-4 border-b border-milktea-100 flex justify-between items-center shrink-0"
        >
          <h2 class="font-bold text-milktea-900 text-lg">頁籤顯示設定</h2>
          <button
            class="material-icons text-milktea-400"
            (click)="closeFeatureDrawer()"
          >
            close
          </button>
        ...
        <div class="p-4 flex-1 overflow-y-auto">
          <div class="flex flex-col gap-3">
            <label
              class="flex items-center justify-between p-3 bg-milktea-50 rounded-xl border border-milktea-100"
              (click)="$event.stopPropagation()"
            >
              <span class="font-bold text-milktea-900">月曆</span>
              <input
                type="checkbox"
                [checked]="featureTabs.includes('month')"
                (change)="toggleFeatureTab('month')"
                (click)="$event.stopPropagation()"
                class="w-5 h-5 accent-milktea-600"
              />
            </label>
            <label
              class="flex items-center justify-between p-3 bg-milktea-50 rounded-xl border border-milktea-100"
            ...
            >
              ...
              <input
                type="checkbox"
                [checked]="featureTabs.includes('week')"
                (change)="toggleFeatureTab('week')"
                (click)="$event.stopPropagation()"
                class="w-5 h-5 accent-milktea-600"
              />
            </label>
            ...
            >
              ...
              <input
                type="checkbox"
                [checked]="featureTabs.includes('day')"
                (change)="toggleFeatureTab('day')"
                (click)="$event.stopPropagation()"
                class="w-5 h-5 accent-milktea-600"
              />
            </label>
            ...
            >
              ...
              <input
                type="checkbox"
                [checked]="featureTabs.includes('monthApple')"
                (change)="toggleFeatureTab('monthApple')"
                (click)="$event.stopPropagation()"
                class="w-5 h-5 accent-milktea-600"
              />
            </label>
      ...
      <div
        ...
        (click)="$event.stopPropagation()"
        [style.transform]="
          showCategoryDrawer ? 'translateY(0)' : 'translateY(100%)'
        "
      >
        <div
          class="p-4 border-b border-milktea-100 flex justify-between items-center shrink-0"
        ...
        <div
          class="h-[50vh] overflow-y-auto px-4 py-4 overflow-x-hidden flex flex-col"
          cdkDropList
          (cdkDropListDropped)="onCategoryDrop($event)"
        >
          <div
            *ngIf="categories.length === 0"
            class="text-center text-milktea-400 text-sm py-4"
          >
            目前沒有分類
          </div>
          <div
          ...
          >
            <div
              *cdkDragPreview
              class="w-4 h-4 rounded-full bg-red-400 shadow-sm shadow-red-500/50"
            ></div>
            <div class="flex items-center gap-3">
              <div
                cdkDragHandle
                class="material-icons text-milktea-400 cursor-grab px-1 touch-none"
                (click)="$event.stopPropagation()"
              >
                drag_indicator
              </div>
              <span class="material-icons text-milktea-600">{{
                cat.icon
              }}</span>
              <span class="font-bold text-milktea-800">{{ cat.name }}</span>
            </div>
            <button
              class="material-icons text-red-400 text-sm p-2"
              (click)="deleteCategory($event, cat); $event.stopPropagation()"
            >
              delete
            </button>
          </div>
        ...
        <div
          class="p-4 border-t border-milktea-100 shrink-0 pb-safe z-10 bg-white"
        >
          <button
            class="w-full bg-milktea-600 text-white font-bold py-3 rounded-full shadow-sm"
      ...
      >
        ...
        ></div>
        <div
          class="bg-white rounded-2xl p-6 shadow-xl w-full max-w-sm relative z-10"
        >
          <h3 class="font-bold text-lg text-milktea-900 mb-4">
            {{ editingCategory?.id ? '編輯分類' : '新增分類' }}
          </h3>

          <div class="mb-4">
            <label class="block text-xs font-bold text-milktea-500 mb-1"
              >分類名稱</label
            >
            <input
              type="text"
              [(ngModel)]="editName"
              class="w-full border-b-2 border-milktea-200 focus:border-milktea-500 outline-none py-1 text-milktea-900 font-medium"
              placeholder="例如：工作、運動、家庭"
            />
          </div>

          <div class="mb-6">
            <label class="block text-xs font-bold text-milktea-500 mb-2"
              >選擇圖示</label
            >
            <div class="flex flex-wrap gap-2 max-h-32 overflow-y-auto p-1">
              <button
                *ngFor="let icon of availableIcons"
                ...
                [class.border-milktea-600]="editIcon === icon"
                [class.bg-milktea-100]="editIcon === icon"
                [class.text-milktea-700]="editIcon === icon"
                [class.border-milktea-100]="editIcon !== icon"
                [class.text-milktea-400]="editIcon !== icon"
              >
          ...
          <div class="flex justify-end gap-3">
            <button
              class="px-4 py-2 text-milktea-500 font-bold text-sm"
              (click)="closeEditModal()"
            >
              取消
            </button>
            <button
              class="px-4 py-2 bg-milktea-600 text-white rounded-full font-bold text-sm shadow-sm"
              [disabled]="!editName"
              (click)="saveCategory()"
            >
              儲存
...
export class SettingsComponent implements OnInit, OnDestroy {
  ...
  featureTabs: ('month' | 'week' | 'day' | 'monthApple')[] = [
    'month',
    'week',
    'day',
  ];
      ...
      .subscribe((ws) => {
        ...
        if (ws) {
          ...
          if (ws.userPreferences && ws.userPreferences[this.currentUserId]) {
            this.featureTabs = ws.userPreferences[this.currentUserId].tabs || [
              'month',
              'week',
              'day',
            ];
