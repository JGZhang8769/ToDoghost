...
export class MainViewComponent implements OnInit, OnDestroy {
  ...
  get autocompleteTags(): string[] {
    ...
    return this.availableTags.filter(
      (t) =>
        !this.filterTags.includes(t) && t.toLowerCase().includes(lowerInput),
    );
  }
  ...
  get scheduledDrawerTasks() {
    ...
    return this.filteredTasks.filter(
      (t) => t.date === this.scheduledDrawerDate,
    );
  }
    ...
    this.workspaceService.currentWorkspace$
      ...
      .subscribe((ws) => {
        ...
        if (userId && ws.userPreferences && ws.userPreferences[userId]) {
          this.featureTabs = ws.userPreferences[userId].tabs || [
            'month',
            'week',
            'day',
          ];
        ...
        if (
          !this.featureTabs.includes(this.viewMode as any) &&
          this.featureTabs.length > 0
        ) {
    ...
    if (this.filterTags.length > 0) {
      if (this.filterLogicAnd) {
        filtered = filtered.filter((t) =>
          this.filterTags.every((tag) => t.tags?.includes(tag)),
        );
      } else {
        filtered = filtered.filter((t) =>
  ...
  generateDayView() {
    let allForDay = this.filteredTasks.filter(
      (t) => t.date === this.selectedDateStr,
    );
  ...
  dropToTime(event: CdkDragDrop<any>, hour: number, minute: number) {
    ...
    if (task) {
      ...
      const newEndH = Math.floor(newTotalMinutes / 60)
        .toString()
        .padStart(2, '0');
  ...
  saveForm() {
    ...
    if (
      this.formTaskTagInput.trim() &&
      !this.formTaskTags.includes(this.formTaskTagInput.trim())
    ) {
    ...
    if (this.editingTask) {
      this.taskService
        .updateTask(this.editingTask.id, dataToSave)
        .then(() => {
          this.isSaving = false;
          this.showForm = false;
        })
  ...
  saveWorkspaceName() {
    if (
      this.isEditingWorkspace &&
      this.currentWorkspace &&
      this.editingWorkspaceName.trim()
    ) {
      this.workspaceService.updateWorkspace(this.currentWorkspace.id, {
        name: this.editingWorkspaceName.trim(),
      });
    }
