  ...
  template: `
    <div class="min-h-screen bg-milktea-50 flex flex-col p-6">
      <div class="flex justify-between items-center mb-8">
        ...
        <button
          (click)="logout()"
          class="text-milktea-600 text-sm font-bold bg-white px-4 py-2 rounded-full shadow-sm"
        >
          更換用戶
        </button>
      </div>
      ...
      <div class="flex-1 overflow-y-auto">
        ...
        <div
          *ngIf="!isLoadingWorkspaces && workspaces.length === 0"
          class="text-center text-milktea-400 mt-10"
        >
          您還沒有任何空間，請建立或加入一個。
        </div>
        ...
        <div *ngIf="!isLoadingWorkspaces">
          ...
          >
            <div class="flex justify-between items-center mb-2">
              <h2 class="text-lg font-bold text-milktea-900">
                {{ ws.name || '未命名空間' }}
              </h2>
              <span
                class="text-xs bg-milktea-100 text-milktea-800 px-2 py-1 rounded-full"
                >{{ ws.users.length }}/2 人</span
              >
            </div>
            <div
              class="text-sm text-milktea-500 font-mono bg-milktea-50 p-2 rounded inline-block"
              *ngIf="ws.inviteCode"
            >
              邀請碼:
              <span class="font-bold text-milktea-700">{{
                ws.inviteCode
              }}</span>
            </div>
          </div>
      ...
      <div class="mt-auto pt-6 flex flex-col gap-3">
        <button
          class="w-full bg-milktea-600 text-white font-bold py-4 rounded-2xl shadow-sm active:scale-[0.98]"
          (click)="showNewWorkspaceForm = true"
        >
          + 建立新空間
        </button>
        <div class="flex gap-2">
          <input
            ...
            placeholder="輸入邀請碼"
            class="flex-1 bg-white border border-milktea-200 rounded-2xl px-4 py-3 focus:outline-none focus:border-milktea-400 text-center font-mono placeholder:font-sans"
          />
          <button
            class="bg-milktea-200 text-milktea-900 font-bold px-6 py-3 rounded-2xl active:scale-[0.98]"
            (click)="joinWorkspace()"
      ...
      >
        <div class="bg-white rounded-3xl p-6 w-full max-w-sm shadow-xl">
          ...
          <input
            ...
            placeholder="輸入空間名稱"
            class="w-full bg-milktea-50 border border-milktea-200 rounded-xl px-4 py-3 mb-6 focus:outline-none focus:border-milktea-400 transition-colors"
          />
          <div class="flex gap-3">
            <button
              class="flex-1 py-3 rounded-xl bg-milktea-100 text-milktea-800 font-bold"
              (click)="showNewWorkspaceForm = false"
            >
              取消
            </button>
            <button
...
export class WorkspaceListComponent implements OnInit {
  ...
  async joinWorkspace() {
    ...
    const success = await this.workspaceService.joinWorkspaceByCode(
      this.currentUser.id,
      this.inviteCodeInput.trim().toUpperCase(),
    );
    if (success) {
